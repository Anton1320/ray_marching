    ...
    while let Some(event) = window.next() {
        ...
        if let Event::Input(i) = &event{
            if let Input::Button(j) = i {
                if let ButtonState::Press = j.state {
                    cam.move_pos(j.button);
                }
            }
        }
        for i in 0..cam.screen_resolution.0 {
            for j in 0..cam.screen_resolution.1 {
                ...
                for _k in 0..50 {
                    ...
                    if dist < 0.01 {
                        let a = super_sphere.s.color*((super_sphere.get_normal(cam.pos+ray)*(light-cam.pos-ray).norm()).powf(2.)+0.1);
