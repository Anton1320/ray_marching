    ...
    while let Some(event) = window.next() {
        ...
        for i in 0..cam.screen_resolution.0 {
            for j in 0..cam.screen_resolution.1 {
                ...
                for _k in 0..50 {
                    //ray.z = (ray.z-2.)%4.+2.;
                    let dist = cube.get_distance(cam.pos+ray).max(-sphere.get_distance(cam.pos+ray));
                    if dist < 0.01 {
                        let a: Vector3;
                        if -sphere.get_distance(cam.pos+ray) > cube.get_distance(cam.pos+ray)
                            {a = cube.color*((cube.get_normal(cam.pos+ray)*(light-cam.pos-ray).norm()).powf(2.)+0.2);}
                        else {a = cube.color*((sphere.get_normal(cam.pos+ray)*(-1.)*(light-cam.pos-ray).norm()).powf(2.)+0.2);}
        ...
        //let new = Instant::now();
        //println!("coutnt {:?}", new.duration_since(now));
        ...
        //let now = Instant::now();
        //println!("draw {:?}", now.duration_since(new));
    }
}